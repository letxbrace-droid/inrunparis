      ...
      >
          ...
          <div className="flex items-center gap-4">
            ...
            >
              <span
                ...
                style={{ fontSize: 26, color: th.inkFull, fontFamily: "'Inter', sans-serif", letterSpacing: '-.01em' }}
              >
                N
              </span>
              <span
            ...
            <div className="flex-1 min-w-0">
              <p className="font-bold text-[17px]" style={{ color: th.inkFull }}>Nourdine</p>
              <p className="text-xs mt-0.5" style={{ color: th.inkMuted }}>
                Chauffeur VTC certifié · Paris
              </p>
              <div className="flex items-center gap-0.5 mt-2">
                {[1,2,3,4,5].map(i => (
                  <svg key={i} width="11" height="11" viewBox="0 0 24 24" fill={th.inkLow} className="flex-shrink-0">
                    <polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/>
                  </svg>
                ))}
                <span className="text-xs font-semibold ml-1.5" style={{ color: th.inkMuted }}>5.0</span>
              </div>
            </div>
          </div>

          <div className="mt-4 mb-3.5" style={{ height: 1, background: th.divider }} />
          ...
          <div className="flex gap-2 flex-wrap">
            {['EVTC', '24h/7j', 'Assurance pro'].map(tag => (
              <span
                ...
                style={{ background: th.bgInput, color: th.inkMuted }}
              >
                {tag}
              </span>
            ))}
        ...
        <div className="flex justify-center mb-6">
          <span
            ...
            style={{ fontSize: 22, color: th.inkFull, letterSpacing: '.04em' }}
          >
            {FORMATTED_PHONE}
          </span>
        </div>
        ...
        >
          ...
          <span className="font-semibold text-[15px]" style={{ color: th.inkHigh }}>
            Écrire sur WhatsApp
          </span>
        </a>
        ...
        <div className="flex flex-col gap-4 px-1">
          {REASSURANCE.map(({ label, icon }) => (
            <div key={label} className="flex items-center gap-3">
              <span style={{ color: th.inkDim, flexShrink: 0 }}>{icon}</span>
              <span className="text-sm leading-snug" style={{ color: th.inkMuted }}>{label}</span>
            </div>
          ))}
        </div>
